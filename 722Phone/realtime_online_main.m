...
%     num2str(toc)
    ...
    pearson_csi(mod(packet_index,60)+1,:) = csi_pearson(csi_tx1, csi_tx2);
    
%%%%%%%%%count packets%%%%%%%%%%%%%%%%%%%%    
    if 	mod(packet_index,60) == 0
        std_pearson = mad(pearson_csi,1);
        if std_pearson<0.18
            N_count = N_count + 1;
        else
            Y_count = Y_count + 1;
...
% fprintf(N_count/(packet_index/60))
